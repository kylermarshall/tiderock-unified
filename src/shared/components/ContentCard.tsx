...
export function ContentCard({ label, content, variant = 'default', defaultExpanded = true }: ContentCardProps) {
  ...
  const bgClass = variant === 'highlight' ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-200'
        ...
        <div className="px-4 pb-4">
          {Array.isArray(content) ? (
            <div className="space-y-2">
              {content.map((item, i) => (
                <div key={i} className="text-sm text-slate-700 leading-relaxed p-2 bg-white/70 rounded-lg border border-slate-100">{item}</div>
              ))}
            </div>
          ) : (
