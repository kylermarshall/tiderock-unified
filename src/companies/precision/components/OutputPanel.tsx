...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-pam-charcoal border border-pam-border rounded-lg p-4">
      <h3 className="text-xs font-semibold text-pam-muted mb-3 uppercase tracking-wider">{title}</h3>
      {children}
    </div>
  )
}
    ...
    <ul className="space-y-1.5">
      {items.map((item, i) => (
        <li key={i} className="text-sm text-gray-300 flex gap-2">
          <span className="text-pam-muted mt-0.5 flex-shrink-0 text-xs">▸</span>
          <span>{item}</span>
        </li>
      ))}
    </ul>
    ...
    <div className="space-y-4">
      <Section title="Full Post">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t border-pam-border">
            ...
            <CopyButton text={fullPost.body} label="Body Only" />
          </div>
        </div>
      </Section>
        ...
        <ol className="space-y-2">
          {(postIdeas as PostIdea[]).map((idea, i) => (
            <li key={i} className="flex gap-2">
              <span className="text-pam-muted font-mono text-xs mt-0.5 w-5 flex-shrink-0">{i + 1}.</span>
              <div>
      ...
      <Section title="30-Day Content Calendar">
        <div className="space-y-1.5 max-h-72 overflow-y-auto scrollbar-thin pr-1">
          {(calendar as CalendarEntry[]).map((entry) => (
            <div key={entry.day} className="flex gap-3 text-sm border-b border-pam-border pb-1.5 last:border-0">
              <span className="text-pam-muted font-mono text-xs w-12 flex-shrink-0 pt-0.5">Day {entry.day}</span>
              <div className="flex-1">
                <p className="text-gray-300 text-xs font-medium">{entry.topic}</p>
                <p className="text-gray-500 text-xs mt-0.5 leading-relaxed">{entry.hook}</p>
                <p className="text-pam-muted text-xs mt-0.5 italic">{entry.cta}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-3">
          <CopyButton
            text={(calendar as CalendarEntry[]).map(e => `Day ${e.day} | ${e.topic}\nHook: ${e.hook}\nCTA: ${e.cta}`).join('\n\n')}
            label="Copy Full Calendar"
          />
        </div>
      </Section>
      ...
      <Section title="Title + Thumbnail">
        <div className="space-y-3">
          <div>
            ...
            <div className="mt-2"><CopyButton text={title} label="Copy Title" /></div>
          </div>
          <div>
            <p className="text-xs text-gray-500 mb-1">Thumbnail Text</p>
            <div className="p-3 bg-pam-dark rounded border border-pam-border">
              <p className="text-base font-bold text-white uppercase tracking-wide">{thumbnailText}</p>
            </div>
            <div className="mt-2"><CopyButton text={thumbnailText} label="Copy Thumbnail Text" /></div>
          </div>
        </div>
      </Section>

      <Section title="3-Second Hook">
        <p className="text-sm text-gray-300 mb-2 leading-relaxed">{threeSecondHook}</p>
        <CopyButton text={threeSecondHook} label="Copy Hook" />
      </Section>

      <Section title="Full Script / Outline">
        <div className="space-y-3">
          <div>
            <p className="text-xs text-gray-500 mb-1">Opening Line</p>
            <p className="text-sm text-gray-300 leading-relaxed">{openingLine}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-300 leading-relaxed">{businessInsight}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500 mb-1">CTA</p>
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t border-pam-border">
            ...
            <CopyButton text={scriptOrOutline} label="Script Only" />
          </div>
        </div>
      </Section>
    ...
    <div className="space-y-4">
      <Section title={thread ? 'Thread' : 'Post'}>
        {thread ? (
          <div className="space-y-3">
            {thread.map((tweet, i) => (
              <div key={i} className="p-3 bg-pam-dark rounded border border-pam-border">
          ...
          <div className="space-y-2">
            <p className="text-sm text-gray-300 leading-relaxed whitespace-pre-wrap">{mainPost}</p>
      ...
      <Section title="Alternate Hook">
        <p className="text-sm text-gray-300 mb-2 leading-relaxed">{alternateHook}</p>
      ...
      <Section title="Engagement Question">
        <p className="text-sm text-gray-300 mb-2 leading-relaxed">{engagementQuestion}</p>
        <CopyButton text={engagementQuestion} label="Copy Question" />
      </Section>
...
function InstagramView({ output }: { output: InstagramOutput }) {
  ...
  return (
    <div className="space-y-4">
      <Section title="Visual Hook + On-Screen Text">
        <div className="space-y-3">
          <div>
            <p className="text-xs text-gray-500 mb-1">Visual Hook</p>
            <p className="text-sm text-gray-300 leading-relaxed">{visualHook}</p>
          </div>
          <div>
            <p className="text-xs text-gray-500 mb-1">On-Screen Text</p>
            <div className="p-3 bg-pam-dark rounded border border-pam-border">
              <p className="text-sm font-semibold text-white">{onScreenText}</p>
            </div>
            <div className="mt-2 flex gap-2">
              <CopyButton text={visualHook} label="Copy Hook" />
              <CopyButton text={onScreenText} label="Copy On-Screen Text" />
            </div>
          </div>
        </div>
      </Section>

      <Section title={isCarousel ? 'Carousel Slides' : 'Script / Story'}>
        {isCarousel ? (
          <div className="space-y-2">
            {scriptOrSlides.map((slide, i) => (
              <div key={i} className="p-3 bg-pam-dark rounded border border-pam-border">
                <p className="text-xs text-pam-muted mb-1">Slide {i + 1}</p>
      ...
      <Section title="Caption + CTA">
        <p className="text-sm text-gray-300 leading-relaxed whitespace-pre-wrap mb-3">{caption}</p>
        <div className="flex flex-wrap gap-2 pt-2 border-t border-pam-border">
          <CopyButton text={caption} label="Copy Caption" />
          <CopyButton text={cta} label="Copy CTA" />
          <CopyButton text={`${caption}\n\n${cta}`} label="Copy Caption + CTA" />
        </div>
      </Section>

      <Section title="Suggested Visuals / B-Roll">
    ...
    <div className="space-y-4">
      <Section title="Post">
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-pam-border">
          ...
          <CopyButton text={cta} label="CTA Only" />
          <CopyButton text={`${primaryPost}\n\n${cta}`} label="Copy Post + CTA" />
        </div>
      </Section>

      <Section title="Practical Takeaway">
        <p className="text-sm text-gray-300 leading-relaxed mb-2">{practicalTakeaway}</p>
        <CopyButton text={practicalTakeaway} label="Copy Takeaway" />
      </Section>

      <Section title="Discussion Question">
        <p className="text-sm text-gray-300 leading-relaxed mb-2">{discussionQuestion}</p>
        <CopyButton text={discussionQuestion} label="Copy Question" />
      </Section>
...
export default function OutputPanel({ platform, output, onRegenerate, onTransformTo }: Props) {
  if (!output) {
    return (
      <div className="flex flex-col items-center justify-center h-72 text-center bg-pam-charcoal border border-pam-border rounded-lg">
        <div className="w-14 h-14 rounded-full bg-pam-dark border border-pam-border flex items-center justify-center mb-4">
          <svg className="w-6 h-6 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
        </div>
        <p className="text-sm text-gray-500 font-medium">Select your settings and click Generate Content</p>
        <p className="text-xs text-gray-600 mt-1">Content will appear here — ready to copy and deploy</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-pam-charcoal border border-pam-border rounded-lg p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {others.map(p => (
            <button
              ...
              className="px-3 py-1.5 text-xs font-medium bg-pam-steel text-gray-300 rounded hover:bg-pam-accent hover:text-white transition-colors capitalize"
            >
              → {p}
            </button>
          ))}
