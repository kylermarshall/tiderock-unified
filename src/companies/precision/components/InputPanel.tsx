...
const LINKEDIN_FORMATS = [
  ...
  { value: 'calendar', label: '30-Day Calendar' },
]
  ...
  return (
    <div>
      ...
      <select
        ...
        className="w-full bg-pam-steel border border-pam-border text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-pam-accent"
      >
        {options.map(o => (
...
export default function InputPanel({ platform, inputs, onChange, onGenerate }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <button
        onClick={onGenerate}
        className="w-full py-3 bg-pam-accent hover:bg-blue-500 text-white font-semibold rounded-lg transition-colors text-sm"
      >
        Generate Content
      </button>
    </div>
