...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-proactive-charcoal border border-proactive-border rounded-lg p-4">
      <h3 className="text-xs font-semibold text-proactive-bluelight mb-3 uppercase tracking-wider">{title}</h3>
      {children}
    </div>
  )
}
...
function BulletList({ items }: { items: string[] }) {
  return (
    <ul className="space-y-1.5">
      {items.map((item, i) => (
        <li key={i} className="text-sm text-proactive-text flex gap-2">
          <span className="text-proactive-bluelight mt-0.5 flex-shrink-0 text-xs">▸</span>
          <span>{item}</span>
        </li>
      ))}
    </ul>
    ...
    <div className="space-y-4">
      <Section title="Full Post">
        <div className="space-y-3">
          <div>
            <p className="text-xs text-proactive-muted mb-1">Hook</p>
            <p className="text-sm text-proactive-text leading-relaxed">{fullPost.hook}</p>
          </div>
          <div>
            <p className="text-xs text-proactive-muted mb-1">Body</p>
            <p className="text-sm text-proactive-text whitespace-pre-wrap leading-relaxed">{fullPost.body}</p>
          </div>
          <div>
            <p className="text-xs text-proactive-muted mb-1">Business Impact</p>
            <p className="text-sm text-proactive-text leading-relaxed">{fullPost.businessImpact}</p>
          </div>
          <div>
            <p className="text-xs text-proactive-muted mb-1">CTA</p>
            <p className="text-sm text-proactive-text">{fullPost.cta}</p>
          </div>
          <div className="flex flex-wrap gap-2 pt-2 border-t border-proactive-border">
        ...
        <ol className="space-y-2">
          {(postIdeas as PostIdea[]).map((idea, i) => (
            <li key={i} className="flex gap-2">
              <span className="text-proactive-bluelight font-mono text-xs mt-0.5 w-5 flex-shrink-0">{i + 1}.</span>
              <div>
                <p className="text-sm text-proactive-text">{idea.title}</p>
                <p className="text-xs text-proactive-muted mt-0.5">{idea.angle}</p>
              </div>
            </li>
          ))}
        </ol>
        ...
        <div className="space-y-1.5 max-h-64 overflow-y-auto">
          {(calendar as CalendarEntry[]).map((entry) => (
            <div key={entry.day} className="flex gap-3 text-sm">
              <span className="text-proactive-bluelight font-mono text-xs w-12 flex-shrink-0 pt-0.5">Day {entry.day}</span>
              <div>
                <p className="text-proactive-text text-xs leading-relaxed">{entry.topic}</p>
                <p className="text-proactive-muted text-xs">{entry.hook}</p>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="space-y-4">
      <Section title="Title">
        <div className="flex items-start justify-between gap-2">
          <p className="text-sm text-proactive-text font-medium">{title}</p>
          <CopyButton text={title} />
        </div>
      </Section>

      <Section title="Thumbnail Text">
        <div className="p-3 bg-proactive-dark rounded border border-proactive-border mb-2">
      ...
      <Section title="3-Second Hook">
        <p className="text-sm text-proactive-text mb-2">{threeSecondHook}</p>
      ...
      <Section title="Full Script">
        <div className="space-y-3">
          <div>
            <p className="text-xs text-proactive-muted mb-1">Opening Line</p>
            <p className="text-sm text-proactive-text">{openingLine}</p>
          </div>
          <div>
            <p className="text-xs text-proactive-muted mb-1">Script / Outline</p>
            <p className="text-sm text-proactive-text whitespace-pre-wrap leading-relaxed">{scriptOrOutline}</p>
          </div>
          <div>
            <p className="text-xs text-proactive-muted mb-1">Business Insight</p>
            <p className="text-sm text-proactive-text">{businessInsight}</p>
          </div>
          <div>
            <p className="text-xs text-proactive-muted mb-1">CTA</p>
            <p className="text-sm text-proactive-text">{cta}</p>
          </div>
          <div className="flex flex-wrap gap-2 pt-2 border-t border-proactive-border">
    ...
    <div className="space-y-4">
      <Section title={thread ? 'Thread' : 'Post'}>
        {thread ? (
          <div className="space-y-3">
            {thread.map((tweet, i) => (
              <div key={i} className="p-3 bg-proactive-dark rounded border border-proactive-border">
                <p className="text-xs text-proactive-muted mb-1">{i + 1}/{thread.length}</p>
                <p className="text-sm text-proactive-text">{tweet}</p>
                <div className="mt-2">
          ...
          <div className="space-y-2">
            <p className="text-sm text-proactive-text leading-relaxed">{mainPost}</p>
      ...
      <Section title="Alternate Hook">
        <p className="text-sm text-proactive-text mb-2">{alternateHook}</p>
        <CopyButton text={alternateHook} label="Copy Hook" />
      </Section>

      <Section title="CTA">
        <p className="text-sm text-proactive-text mb-2">{cta}</p>
        <CopyButton text={cta} label="Copy CTA" />
      </Section>

      <Section title="Engagement Question">
        <p className="text-sm text-proactive-text mb-2">{engagementQuestion}</p>
  ...
  return (
    <div className="space-y-4">
      <Section title="Visual Hook">
        <p className="text-sm text-proactive-text mb-2">{visualHook}</p>
        ...
        {isCarousel ? (
          <div className="space-y-2">
            {scriptOrSlides.map((slide, i) => (
              <div key={i} className="p-3 bg-proactive-dark rounded border border-proactive-border">
                <p className="text-xs text-proactive-bluelight mb-1">Slide {i + 1}</p>
                <p className="text-sm text-proactive-text">{slide}</p>
              </div>
            ))}
          ...
          <div className="space-y-2">
            <p className="text-sm text-proactive-text whitespace-pre-wrap leading-relaxed">{scriptOrSlides[0]}</p>
      ...
      <Section title="Caption">
        <p className="text-sm text-proactive-text leading-relaxed mb-2">{caption}</p>
...
function FacebookView({ output }: { output: FacebookOutput }) {
  ...
  return (
    <div className="space-y-4">
      <Section title="Post">
        <p className="text-sm text-proactive-text whitespace-pre-wrap leading-relaxed mb-3">{primaryPost}</p>
        <div className="flex flex-wrap gap-2 pt-2 border-t border-proactive-border">
      ...
      <Section title="Practical Takeaway">
        <p className="text-sm text-proactive-text mb-2">{practicalTakeaway}</p>
        <CopyButton text={practicalTakeaway} />
      </Section>

      <Section title="Discussion Question">
        <p className="text-sm text-proactive-text mb-2">{discussionQuestion}</p>
...
export default function OutputPanel({ platform, output, onRegenerate, onTransformTo }: Props) {
  if (!output) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center bg-proactive-charcoal border border-proactive-border rounded-lg">
        <div className="w-12 h-12 rounded-full bg-proactive-dark border border-proactive-border flex items-center justify-center mb-4">
          <svg className="w-5 h-5 text-proactive-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
        </div>
        <p className="text-sm text-proactive-muted">Select your settings and click Generate Content</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-proactive-charcoal border border-proactive-border rounded-lg p-4 space-y-3">
        <p className="text-xs font-semibold text-proactive-muted uppercase tracking-wider">Regenerate Variation</p>
        <RegenerateButtons onRegenerate={onRegenerate} />
      </div>

      <div className="bg-proactive-charcoal border border-proactive-border rounded-lg p-4 space-y-3">
        <p className="text-xs font-semibold text-proactive-muted uppercase tracking-wider">Turn Into Another Platform</p>
        <div className="flex flex-wrap gap-2">
          {others.map(p => (
            <button
              key={p}
              onClick={() => onTransformTo(p)}
              className="px-3 py-1.5 text-xs font-medium bg-proactive-steel text-proactive-muted rounded hover:bg-proactive-blue hover:text-white transition-colors capitalize"
            >
              → {p}
            </button>
          ))}
