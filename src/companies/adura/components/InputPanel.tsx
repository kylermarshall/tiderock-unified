import type {
  Platform, PlatformInputs, ProblemType, ToneType,
  LinkedInInputs, YouTubeInputs, TwitterInputs, InstagramInputs, FacebookInputs
} from '../types'
import { TARGET_AUDIENCES, CONTENT_ANGLES, OBJECTIVES, PROBLEM_LABELS } from '../lib/companyProfile'
...
export function InputPanel({ platform, inputs, onChange, onGenerate, isGenerating }: InputPanelProps) {
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5 sticky top-6">
      ...
      <Field label="Operational Problem">
        <select
          ...
          onChange={e => update('problem', e.target.value as ProblemType)}
        >
      ...
      <Field label="Tone">
        <select
          className={selectClass}
          value={inputs.tone}
          onChange={e => update('tone', e.target.value as ToneType)}
        >
          {tones.map(t => (
      ...
      {platform === 'linkedin' && (
        <Field label="Content Focus">
          <select
            ...
            onChange={e => update('contentFormat', e.target.value)}
          >
      ...
      {platform === 'twitter' && (
        <Field label="Post Format">
          <select
            ...
            onChange={e => update('postFormat', e.target.value)}
          >
      ...
      {platform === 'instagram' && (
        <>
          ...
          <Field label="Visual Style">
            <select
              className={selectClass}
              value={(inputs as InstagramInputs).visualStyle}
