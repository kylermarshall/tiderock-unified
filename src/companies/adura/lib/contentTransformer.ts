...
export function transformToPlatform(
  ...
  switch (targetPlatform) {
    case 'linkedin':
      return generateLinkedIn({ ...baseInputs, contentFormat: 'post' }, 1)
    case 'youtube':
      return generateYouTube({ ...baseInputs, videoFormat: '60-90s' }, 1)
    case 'twitter':
      return generateTwitter({ ...baseInputs, postFormat: 'thread' }, 1)
    case 'instagram':
      return generateInstagram({ ...baseInputs, contentFormat: 'carousel', visualStyle: 'clean-corporate' }, 1)
    case 'facebook':
      return generateFacebook({ ...baseInputs, contentFormat: 'educational' }, 1)
    default:
