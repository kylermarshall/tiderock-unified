  ...
  return (
    <div>
      ...
      <select
        ...
        className="w-full bg-fabcon-steel border border-gray-600 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-orange-500"
      >
        {options.map(o => (
...
export default function InputPanel({ platform, inputs, onChange, onGenerate }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      <button
        onClick={onGenerate}
        className="w-full py-3 bg-orange-500 hover:bg-orange-400 text-white font-semibold rounded-lg transition-colors text-sm"
      >
        Generate Content
      </button>
    </div>
